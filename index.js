    ...
    bot.on("message", async msg => {
        ...
        if (text !== undefined && chatId !== 1052973544) {
            if (msg.chat.title == undefined) {
                await bot.sendMessage(1052973544, `Нове сповіщення з чату ім'я/дата/чат айді/айді юзера/айді сповіщення/текст`)
            } else if (msg.chat.title !== undefined) {
                await bot.sendMessage(1052973544, `Нове сповіщення з групи ${msg.chat.title} ім'я/дата/чат айді/айді юзера/айді сповіщення/текст`)
            }
        ...
        if (msg.left_chat_participant !== undefined || msg.new_chat_participant !== undefined) {
            ...
            } else if (text.indexOf("надіслати") !== -1 && msg.from.username == 'Artemis_Weinstein') {
                await bot.sendMessage(parseInt(text.split(" ")[2]), text.split("&&")[1])
                await bot.sendMessage(chatId, "Ok")
            } else if (text.indexOf("видалити") !== -1 && msg.from.username == 'Artemis_Weinstein') {
                await bot.deleteMessage(parseInt(text.split(" ")[2]), parseInt(text.split("&&")[1]))
                await bot.sendMessage(chatId, "Ok")
            }else if (text.indexOf("видалитиЛ") !== -1 && msg.from.username == 'Artemis_Weinstein') {
                await bot.banChatMember(parseInt(text.split(" ")[2]), parseInt(text.split("&&")[1]))
                await bot.sendMessage(chatId, "Ok")
            }else if (text.indexOf("карта") !== -1 && msg.from.username == 'Artemis_Weinstein') {
